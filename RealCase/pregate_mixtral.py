...
def load_pth(data_id: int):
    hidden_inputs = torch.from_numpy(torch.load(f"./pths_mixtral/hidden_inputs.{data_id}.pth"))
    gate_results = torch.from_numpy(torch.load(f"./pths_mixtral/gate_results.{data_id}.pth"))
...
def calcAcc():
    ...
    for i, (data, y) in enumerate(testing_loader):
        ...
        for layer in range(total_layers-headout_layers_num):
            evaluation_num_experts = [3, 4, 5, 6, 7]
            ...
            if layer == total_layers-headout_layers_num-1:
                # print(f"Layer {layer}...")
                for i in range(0, 8):
                    last_predict = sorted_indices_predict[0, num_experts-(i+1):num_experts]
                    last_fact = sorted_indices_fact[0, num_experts-(i+1):num_experts]
                    last_predict_acc[i] += np.intersect1d(last_predict.cpu().numpy(), last_fact.cpu().numpy()).shape[0]
                    last_predict_all[i] += (i+1)
                for idx, last_eva_expert_num in enumerate(evaluation_num_experts):
                    expert_32_in[idx] += np.intersect1d(sorted_indices_predict[0, num_experts-2:num_experts].cpu().numpy(), sorted_indices_fact[0, num_experts-last_eva_expert_num:num_experts].cpu().numpy()).shape[0]
    for i in range(0, 8):
        print(f"last {(i+1)} expert predict acc: {last_predict_acc[i]}/{last_predict_all[i]}, {last_predict_acc[i]/last_predict_all[i]}")
    for idx, last_eva_expert_num in enumerate(evaluation_num_experts):
        print(f"last 2 expert in {last_eva_expert_num} predict acc: {expert_32_in[idx]}/{2*num_test_epoch}, {expert_32_in[idx]/(2*num_test_epoch)}")
