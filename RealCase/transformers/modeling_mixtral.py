...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        self.register(ori_hidden_states, routing_weights)
...
class MixtralDecoderLayer(nn.Module):
    def __init__(self, config: MixtralConfig, layer_idx: int, register):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = MIXTRAL_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)

        self.block_sparse_moe = MixtralSparseMoeBlock(config, register=register)
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def __init__(self, config: MixtralConfig, register):
        ...
        self.layers = nn.ModuleList(
            [MixtralDecoderLayer(config, layer_idx, register) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def __init__(self, config, register):
        super().__init__(config)
        self.model = MixtralModel(config, register=register)
        self.vocab_size = config.vocab_size
